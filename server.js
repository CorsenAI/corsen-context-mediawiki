...
const CX = {
  stack: 'MediaWiki',
  accent: '#155e75',
  title: 'MediaWiki + Corsen Context',
  h1: 'A wiki that talks to AI agents',
  lede: 'This site runs on MediaWiki, the software behind Wikipedia. Corsen Context reads its Action API and hands agents four explicit, read-only tools - over MCP, over WebMCP inside this page, and through llms.txt.',
  noun: 'articles',
  nounSing: 'article',
  postsH2: 'Wiki articles',
  postsIntro:
    'Every card below is a live wiki page. The same records answer search_site, list_content and get_page_content.',
  source: 'The MediaWiki Action API',
  rule: 'public main-namespace pages, fetched in bounded batches with no wiki credential',
  envHint: ', MW_API_URL and a descriptive MW_USER_AGENT',
  repo: 'https://github.com/CorsenAI/corsen-context-mediawiki',
  description:
    'A MediaWiki site made agent-native with Corsen Context: four read-only tools over MCP, WebMCP and llms.txt.',
};
...
${bridgeTag}</head>
<body>
<div data-cc-nav data-stack="MediaWiki" data-uid="mediawiki" data-home="#top" data-accent="#155e75"></div>
...
<section id="live" class="cx-section cx-live">
  ...
  <div data-cc-observatory data-stack="MediaWiki" data-endpoint="/v1/mcp" data-query="MediaWiki" data-accent="#155e75"></div>
</section>
</main>
<footer data-cc-foot data-stack="MediaWiki" data-accent="#155e75"></footer>
<script>/* ============================================================
...
app.get(
  ...
  asyncHandler(async (_req, res) => {
    const wikiPages = await fetchPages();
    res.type('html').send(pageShell(CX.title, cxHome(wikiPages)));
  }),
);
...
app.use(
  asyncHandler(async (req, res, next) => {
    ...
    res
      .type('html')
      .send(pageShell(page.title, cxArticle(page), page.description || CX.description));
  }),
);
